...
Authentication is delegated entirely to an nginx reverse proxy running Authelia
forward-auth (or oauth2-proxy).
The service is auth-agnostic: it trusts the identity headers injected by the
proxy and uses them for ownership and permission checks only.

> **Note:** pic-map currently has no production HTTP server — it targets
> print-ready output (SVG/PDF/EPS). This document describes the authentication
> contract that any future preview server, web editor, or upload endpoint must
> follow.
...
nginx  (TLS termination, rate limiting)
  ...
  ▼
pic-map service  (binds to 127.0.0.1:3000)
```

**Key invariants:**

* nginx MUST strip any incoming `Remote-User`, `Remote-Name`, `Remote-Email`, and
  `Remote-Groups` headers sent by untrusted clients **before** forwarding them to
  the application.
* The service binds to `127.0.0.1` in production so it cannot be reached without
  going through the proxy.
* If a request reaches the service without the expected identity headers, it is
  processed as the built-in guest user. The guest user has no group memberships
  and therefore no ownership privileges; protected endpoints (those wrapped in
  `requireAuth`) will still reject the request with `401`.
* Deployments that prefer to reject unauthenticated requests outright as
  defense-in-depth can opt in to the `requireProxyAuth` middleware
  (see `src/auth/proxyAuth.ts`).
...
|-----------------|----------------|-----------------------------------------------------------------|
| `Remote-User`   | Authelia/nginx | Primary identity — the user's email address                     |
...
4. Guest sentinel — used when none of the above are present

The service reads these headers in `src/auth/proxyAuth.ts` via `resolveUserFromHeaders`.
All emails from headers are normalised to lowercase before use.
...
```nginx
# /etc/nginx/sites-enabled/pic-map.conf
...
server {
    ...
    server_name pic-map.example.com;

    # TLS configuration (Let's Encrypt / your CA)
    ssl_certificate     /etc/letsencrypt/live/pic-map.example.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/pic-map.example.com/privkey.pem;
...
access_control:
  ...
  rules:
    # Allow all authenticated users to access the pic-map service
    - domain: pic-map.example.com
...
If downstream services require a valid OIDC access token (e.g., to call a
protected API on behalf of the user), deploy oauth2-proxy in front of the
service alongside (or instead of) Authelia.

```
nginx → oauth2-proxy → Dex (OIDC IdP) → pic-map service
```

oauth2-proxy injects the access token in the `X-Forwarded-Access-Token` header
after authentication.

> **Important**: Never log the access token. The audit logging in this service
> logs only the principal email (`Remote-User`), never tokens or credentials.
...
`DEV_STUB_USER` is **silently ignored** when `NODE_ENV=production` or
`REQUIRE_PROXY_AUTH=true`.

If neither proxy headers nor `DEV_STUB_USER` are present, the request is
processed with the `guest` sentinel identity, which has no group memberships
and therefore no ownership or admin privileges.
...
- [ ] The service is bound to `127.0.0.1` (not `0.0.0.0`)
